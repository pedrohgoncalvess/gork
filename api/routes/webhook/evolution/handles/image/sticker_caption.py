...
def add_caption_to_image(img: Image.Image, caption_text: str = "") -> Image.Image:
    ...
    if top_text:
        _draw_meme_text(draw, top_text.upper(), width, height, position="top")

    if bottom_text:
        _draw_meme_text(draw, bottom_text.upper(), width, height, position="bottom")

    return img


def _draw_meme_text(draw, text: str, width: int, height: int, position: str = "top"):
